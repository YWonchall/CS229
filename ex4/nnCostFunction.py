...
#               first time.
    ...
    for k in range(m):
        z2 = np.dot(X[k],Theta1.T)
        a2 = sigmoid(z2)
        a2 = np.concatenate(([1],a2))
        a3 = sigmoid(np.dot(a2,Theta2.T))
        delta3 = a3-y[k]
        delta2 = np.dot(Theta2.T,delta3)*sigmoidGradient(np.concatenate(([1],z2)))
        Delta1 = Delta1 + np.dot(delta2.reshape(-1,1),X[k].reshape(1,-1))[1:,:]
        Delta2 = Delta2 + np.dot(delta3.reshape(-1,1),a2.reshape(1,-1))
...
#
    reg = np.sum(Theta1[:,1:]*Theta1[:,1:]) + np.sum(Theta2[:,1:]*Theta2[:,1:])
    J += Lambda*reg/m/2
